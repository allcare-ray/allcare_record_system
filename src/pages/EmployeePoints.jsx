...
const EmployeePoints = () => {
  const { employees, employeePoints, employeePointRecords, addEmployeePoint, updateEmployeePoint, deleteEmployeePoint, batchUpdateEmployeePoints } = useData();
  ...
  // 使用ref来跟踪已处理的员工
  const processedEmployeesRef = useRef(new Set());

  // 自动为每个员工创建积分记录（如果不存在）
  React.useEffect(() => {
    const employeesNeedingPoints = employees.filter(employee =>
      !employeePoints.find(point => point.employeeId === employee.id) &&
      !processedEmployeesRef.current.has(employee.id)
    );

    employeesNeedingPoints.forEach(employee => {
      processedEmployeesRef.current.add(employee.id);
      addEmployeePoint({
  ...
  }, [employees, employeePoints, addEmployeePoint]);

  // 过滤积分记录
  const filteredPoints = employeePoints.filter(point => {
    const employee = employees.find(e => e.id === point.employeeId);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (editingPoint) {
      updateEmployeePoint(editingPoint.id, formData, null, true); // 标记为编辑操作
    } else {
      addEmployeePoint({
        ...formData,
  ...
  const handleBatchPointsSubmit = (e) => {
    ...
    if (batchPointsFormData.pointChange) {
      ...
      // 使用批量调整函数
      batchUpdateEmployeePoints(pointChange, batchPointsFormData.reason || '批量积分调整');
  ...
  const handlePointsSubmit = (e) => {
    e.preventDefault();
    const point = employeePoints.find(p => p.employeeId === showPointsForm);
  ...
  const handleExchangeSubmit = (e) => {
    e.preventDefault();
    const point = employeePoints.find(p => p.employeeId === showExchangeForm);
    const employee = employees.find(e => e.id === showExchangeForm);
    if (point && employee) {
      ...
      // 格式：兑换时间|兑换物品|兑换人|减少积分
      const newExchangeEntry = `${exchangeFormData.exchangeDate}|${exchangeFormData.exchangeContent}|${employee.name}|${pointsUsed}`;
      ...
      // 重置表单
      setExchangeFormData({ exchangeDate: '', exchangeContent: '', pointsUsed: '' });
      setShowExchangeForm(null);
    }
  };

  const openPointsForm = (employeeId) => {
    setShowPointsForm(employeeId);
    setPointsFormData({
      pointChange: '',
      changeType: 'increase',
      reason: '积分调整'
  ...
  const openExchangeForm = (employeeId) => {
    setShowExchangeForm(employeeId);
    setExchangeFormData({
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">员工积分管理</h1>
            <p className="text-gray-600">管理员工积分和兑换记录</p>
          </div>
        </div>
      ...
      <div className="bg-green-50 rounded-lg p-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-green-600">总积分记录</p>
            <p className="text-2xl font-bold text-green-700">{employeePoints.length}</p>
          </div>
          <div>
            <p className="text-sm text-green-600">搜索结果</p>
            <p className="text-2xl font-bold text-green-700">{filteredPoints.length}</p>
          </div>
          <div>
            <p className="text-sm text-green-600">总积分</p>
            <p className="text-2xl font-bold text-green-700">
              {employeePoints.reduce((sum, point) => sum + (parseInt(point.points) || 0), 0)}
            </p>
          </div>
        </div>
      </div>
                          ...
                          <div className="relative">
                            ...
                            {activeDropdown === `${point.id}-points` && (
                              <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-20">
                                <div className="py-2">
                                  <button
                                    onClick={() => handleDropdownAction(() => openPointsForm(point.employeeId), point)}
                                  ...
                                  <button
                                    onClick={() => handleDropdownAction(() => setShowRecords(point.employeeId), point)}
                                    className="w-full px-4 py-3 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center space-x-3"
                                  >
                          ...
                          <div className="relative">
                            ...
                            {activeDropdown === `${point.id}-exchange` && (
                              <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-20">
                                <div className="py-2">
                                  <button
                                    onClick={() => handleDropdownAction(() => openExchangeForm(point.employeeId), point)}
                                    className="w-full px-4 py-3 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center space-x-3"
                                  >
                                  ...
                                  <button
                                    onClick={() => handleDropdownAction(() => setShowExchangeRecords(point.employeeId), point)}
                                    className="w-full px-4 py-3 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center space-x-3"
                                  >
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex space-x-3 pt-4">
                ...
                <button
                  type="submit"
                  className="bg-green-600 hover:bg-green-700 text-white font-medium py-3 px-6 rounded-lg transition-colors duration-200 flex-1"
                >
                  {editingPoint ? '更新' : '添加'}
                </button>
              </div>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              {employeePointRecords
                .filter(record => record.employeeId === showRecords)
                  ...
                  <div key={record.id} className="bg-gray-50 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm mb-3">
                      ...
                      <div>
                        ...
                        <span className="font-medium text-green-600">{record.currentPoints || record.newPoints}</span>
                      </div>
                    </div>
                    {record.reason && (
              ...
              {employeePointRecords.filter(record => record.employeeId === showRecords).length === 0 && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-hidden">
            ...
            <div className="overflow-y-auto" style={{ maxHeight: 'calc(90vh - 120px)' }}>
              {(() => {
                const point = employeePoints.find(p => p.employeeId === showExchangeRecords);
                const employee = employees.find(e => e.id === showExchangeRecords);
                ...
                const records = exchangeRecord.split('\n').filter(record => record.trim());

                return (
                  <div className="space-y-4">
                    <div className="bg-green-50 rounded-lg p-4 mb-4">
                      <h4 className="font-medium text-green-900 mb-2">员工信息</h4>
                      <p className="text-green-700">{employee?.name} - {employee?.phone}</p>
                    </div>

                    {records.map((record, index) => {
                      // 新格式：兑换时间|兑换物品|兑换人|减少积分
                      // 旧格式：兑换时间: 兑换内容
                      let date, content, exchangePerson, pointsUsed;
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-4">
              <p className="text-sm text-orange-800">
                <strong>注意：</strong>此操作将对所有员工的积分进行调整。请谨慎操作。
              </p>
            </div>
